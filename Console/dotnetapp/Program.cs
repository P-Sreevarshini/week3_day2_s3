    ...
    namespace dotnetapp
    {
        ...
        public static void DeleteDepartment(string connectionString, int deptId)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
                ...
                command.ExecuteNonQuery();
                Console.WriteLine($"Deleted 1 department(s).");
            }
        }

            public static bool EmpRecordExists(string connectionString, string tableName, int id)
            {
                bool exists = false;
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
            ...
            public static bool DeptRecordExists(string connectionString, string tableName, int id)
            {
                bool exists = false;
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();
                    string query = $"SELECT COUNT(*) FROM {tableName} WHERE DeptId = @Id"; // Change 'EmpId' to 'DeptId'
                    SqlCommand command = new SqlCommand(query, connection);
                    command.Parameters.AddWithValue("@Id", id);
                    int count = (int)command.ExecuteScalar();
                    exists = count > 0;
